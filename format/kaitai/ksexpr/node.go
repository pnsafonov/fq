...
type LookupFn func(input any, ns string, name string) (any, error)
...
func (i IdentNode) String() string {
	if i.NS.Str != "" {
		return fmt.Sprintf("%s::%s", i.NS.Str, i.Name.Str)
	}
	return i.Name.Str
}
...
func (i IdentNode) Eval(input any) (any, error) {
	if c, ok := input.(Caller); ok {
		v, err := c.KSExprCall(i.NS.Str, i.Name.Str, nil)
		if err != nil {
			return nil, err
		}
		return ToValue(v), nil
	}
	return nil, noSuchKeyError{input, fmt.Sprintf("%s::%s", i.NS.Str, i.Name.Str)}
}
...
func (t TrailerCallNode) Eval(input any) (any, error) {
	...
	if tc, ok := input.(Caller); ok {
		v, err := tc.KSExprCall("", t.Name.Str, av)
		if err != nil {
			return nil, err
		}
		return ToValue(v), nil
